...
class ProxyManager {
    ...
    nextAgent(silent = false) {
        const url = this.getNextProxy(silent);
        return this.getProxyAgent(url);
    }
    ...
    reportFailure() {
        ...
        logger.warn(`ProxyManager tracking failure ${this.failedRequestsCount}/${threshold}`);
