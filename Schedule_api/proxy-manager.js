...
class ProxyManager {
    constructor() {
        ...
        logger.info(`🔧 [Schedule] ProxyManager initialized with List ${this.currentListIndex + 1} (${this.proxyList.length} proxies)`);
    }
    ...
    getNextProxy(silent = false) {
        ...
        if (proxyUrl && !silent) {
            logger.info(`🔄 [Schedule] Using proxy: ${proxyString.split(':')[0]}:${proxyString.split(':')[1]} (Index: ${this.currentIndex === 0 ? this.proxyList.length - 1 : this.currentIndex - 1})`);
        }
        return proxyUrl;
    }
    ...
    getProxyAgent(proxyUrl) {
        ...
        if (this.proxyCache.has(proxyUrl)) return this.proxyCache.get(proxyUrl);
        try {
    ...
    reportFailure() {
        ...
        logger.warn(`⚠️ [Schedule] ProxyManager tracking failure ${this.failedRequestsCount}/${threshold}`);
    ...
    switchList() {
        ...
        logger.info(`🔄 [Schedule] Switched to Proxy List ${this.currentListIndex + 1} (${this.proxyList.length} proxies) after ${threshold} failures`);
    }

    reset() {
        this.currentIndex = 0;
        logger.info('🔄 [Schedule] Proxy rotation reset to start');
    }
}
