...
class ProxyManager {
    constructor() {
        ...
        logger.info(`🔧 [fetch-job] ProxyManager initialized with List ${this.currentListIndex + 1} (${this.proxyList.length} proxies)`);
    }
    ...
    getNextProxy(silent = false) {
        ...
        if (proxyUrl && !silent) {
            logger.info(`🔄 [fetch-job] Using proxy: ${proxyString.split(':')[0]}:${proxyString.split(':')[1]}`);
        }
        return proxyUrl;
    }
    ...
    reportFailure() {
        ...
        logger.warn(`⚠️ [fetch-job] ProxyManager tracking failure ${this.failedRequestsCount}/${threshold}`);
    ...
    reportSuccess() {
        if (this.failedRequestsCount > 0) this.failedRequestsCount = 0;
    }
    ...
    switchList() {
        ...
        logger.info(`🔄 [fetch-job] Switched to Proxy List ${this.currentListIndex + 1} (${this.proxyList.length} proxies) after ${threshold} failures`);
    }
}
